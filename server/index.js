...
app.post('/api/inventory', (req, res) => {
    ...
    db.run(sql, [name, category, potency, batch, stock, uom, minAlert, expiry], function(err) {
        if (err) return res.status(500).json({ error: err.message });
...
app.put('/api/inventory/:id', (req, res) => {
    ...
    db.run(sql, [name, category, potency, batch, stock, uom, minAlert, expiry, req.params.id], function(err) {
        if (err) return res.status(500).json({ error: err.message });
...
app.post('/api/procurements', (req, res) => {
    ...
    db.serialize(() => {
        ...
        db.run(sqlProc, [productId, date, quantity, unitCost, supplier], function(err) {
            if (err) {
                db.run("ROLLBACK");
...
app.get('/api/deliveries', (req, res) => {
    ...
    `, (err, rows) => {
        if (err) return res.status(500).json({ error: err.message });
        const results = rows.map(r => ({ ...r, items: JSON.parse(r.items).filter(i => i.id !== null) }));
        res.json(results);
    });
});
    ...
    db.serialize(() => {
        ...
        db.run(sqlDel, [date, recipient], function(err) {
            ...
            items.forEach(item => {
                ...
                db.run(sqlItem, [deliveryId, item.productId, item.quantity, item.distributionPrice], function(err) {
                    if (err && !hasError) {
                        hasError = true;
                        db.run("ROLLBACK");
                        return res.status(500).json({ error: err.message });
                    ...
                    db.run("UPDATE inventory SET stock = stock - ? WHERE id = ?", [item.quantity, item.productId], function(err) {
                        if (err && !hasError) {
                            hasError = true;
                            db.run("ROLLBACK");
                            return res.status(500).json({ error: err.message });
...
app.post('/api/employees', (req, res) => {
    ...
    db.run(sql, [name, role, contact, salary, status, joinedDate], function(err) {
        if (err) return res.status(500).json({ error: err.message });
        res.json({ id: this.lastID, ...req.body });
    });
});
...
app.put('/api/employees/:id', (req, res) => {
    ...
    db.run(sql, [name, role, contact, salary, status, joinedDate, req.params.id], function(err) {
        if (err) return res.status(500).json({ error: err.message });
        res.json({ id: req.params.id, ...req.body });
    });
});
...
app.get('/api/audit-logs', (req, res) => {
    db.all("SELECT * FROM audit_logs ORDER BY date DESC LIMIT 100", (err, rows) => {
        if (err) return res.status(500).json({ error: err.message });
        res.json(rows);
    });
});
